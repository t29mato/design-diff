...
### 7.4 発見: 沈黙原則の警告表示がSVG本体に無かった(2026-08-29の点検で発見・修正)

HQ指示「既存の警告・エラーメッセージが自己説明的か点検」への対応中に発見した
バグ。`diff.warnings`(サブモジュールのimport失敗)は§7.1でMermaid出力のnoteに
追加していたが、**PRコメントに実際に埋め込まれる画像である`GitHubStyleSvgRenderer`
(`--format svg`の既定)には表示していなかった**。Mermaidブロックは`<details>`
内の折りたたまれたフォールバックに格納されているため(§7.3)、レビュアーが
最初に見る画像だけでは「解析が部分的だった」ことに気付けない――HQが公開前に
直すべきバグと断じた沈黙原則の毒(§14.2で言及した通り)を、SVG移行後にも
再発させてしまっていた。

対応: `GitHubStyleSvgRenderer`に、図の一番上に警告バナー(黄背景、⚠マーク+
スキップしたモジュール名の一覧)を描画する処理を追加した。実装上の要点:

- 既存のレイアウト計算(`_layout()`/`_collect_boxes()`)には一切手を入れず、
  バナーの高さぶん`<g transform="translate(0, banner_height)">`で以降の
  コンテンツ全体をずらすだけにした(既存のクラスボックス配置ロジックの
  再テスト・再検証コストを避けるため)
- **実装直後に発見した抜け(実機検証で発覚)**: バナーの文言(モジュール名を
  含む)がクラスボックスの並びより横に長い場合、キャンバス幅が足りず
  テキストが切れる問題があった。`_warnings_banner_width()`を追加し、
  レイアウト由来の幅とバナー由来の幅の大きい方をキャンバス幅として使う
  よう修正。GitHub実機(スパイクファイル、検証後削除)で、修正前は
  テキストが切れること、修正後は正しく収まることの両方を確認した
- バナーの文言は英語にした(§14.6で決めた「PRコメントに埋め込まれる公開向けの
  文言は英語」という方針を一貫させるため)
...
### 14.6 言語方針(2026-08-29の「自己説明的か点検」で確定)

HQ指示「既存の警告・エラーメッセージが自己説明的か点検」の実施中に、
MCPツールのdocstring(実装時点では日本語のまま書いてしまっていた)が
MCPクライアントに直接見える公開APIであることに気付いた。これを機に、
design-diffが持つテキストを「誰に向けたものか」で分類し、言語方針を明文化する:

| 分類 | 言語 | 理由 |
|---|---|---|
| README・AGENTS.md・llms.txt・SKILL.md | 英語 | Fableが既に英語で公開済み。国際的な発見可能性・利用可能性が目的 |
| MCPツールのdocstring・エラー文言(`analyze_design_diff`) | 英語 | MCPプロトコル経由で任意のクライアントに公開される「公開API」であり、READMEと同じ扱い |
| CLI/GitHub Actionの`--help`(argparse) | 英語 | READMEを読んで試す誰もが最初に見るテキストであり、同じ理由で英語に統一(2026-08-29に修正) |
| **PRコメントに実際に埋め込まれる画像(SVG)のプローズ文言**(例: 警告バナー) | 英語 | design-diffを導入した任意のリポジトリのPRレビュアーが直接見るため(§7.4で追加した警告バナーから、この方針を適用開始) |
| CLI/Actionのエラー詳細メッセージ(`Py2pumlExtractionError.friendly_message()`) | **日本語のまま(未対応・要判断)** | 複数回のHQレビューを経て磨き込まれた文面であり、翻訳は内容変更のリスクを伴う。README等と違い「まっさらな新規コンテンツ」ではないため、今回は自己判断で書き換えず、HQに判断を仰ぐ対象として残した |
| 内部コード コメント・設計ドキュメント(このファイル含む) | 日本語のまま | HQ/メンテナ向けの内部設計ナラティブであり、公開APIではない |

**未解決のまま残した論点**: `friendly_message()`(CLI/Actionが`--format`実行
失敗時に出す詳細ガイダンス)を英語化すべきかどうかは、今回は判断を保留した。
理由は上表の通り、既に磨き込まれた内容の書き換えという性質上、内容変更を
伴わない他の修正(docstring・argparse help・SVGの新規プローズ)とはリスクの
質が異なるため。HQの判断を仰ぐ。
