...
class MermaidRenderer:
    ...
    def _warnings_note(self, warnings: tuple[str, ...]) -> str:
        ...
        「点検」で追記(2026-08-29): モジュール名の一覧だけでは、レビュアーが
        これを見て何をすべきか分からない(自己説明的でない)という指摘に対応し、
        典型的な原因と対処のヒントを1行添えた(詳細な文面はCLI/Actionの
        friendly_message()が既に持っているため、ここでは短い誘導に留める)。
        """
        lines = [
            f"⚠ 解析できなかったモジュール: {len(warnings)}件"
            "(import失敗。多くは対象パッケージ自身の依存不足が原因)"
        ]
